...
class WebGSM_Checkout_Pro {
    ...
    public function addresses_page_content() {
        ...
        // Reindex arrays to ensure sequential indices (0, 1, 2...)
        $addresses = array_values($addresses);
        $companies = array_values($companies);
        $persons = array_values($persons);
        ...
        if ($addresses) {
            echo '<table class="shop_table"><thead><tr><th>Etichetă</th><th>Adresă</th><th>Tel</th><th>Acțiuni</th></tr></thead><tbody>';
            foreach ($addresses as $i => $a) {
                $label = $a['label'] ?? 'Adresa '.($i+1);
                $addr = ($a['address'] ?? '');
                $city = ($a['city'] ?? '');
                $addr_display = $addr . ( ($addr !== '' && $city !== '') ? ', ' : '' ) . $city;
                echo '<tr><td>'.esc_html($label).'</td><td>'.esc_html($addr_display).'</td><td>'.esc_html($a['phone'] ?? '').'</td><td><a href="#" class="edit-saved-address" data-index="'.$i.'">Editează</a> | <a href="#" class="delete-saved-address" data-index="'.$i.'">Șterge</a></td></tr>';
            }
            echo '</tbody></table>';
        } else echo '<p>Nu ai adrese.</p>';
        
        echo '<button type="button" class="btn-add-small" id="add_new_address_btn">+ Adauga adresa</button>'; 

        
        echo '<h3 style="margin-top:30px;">Firme</h3>';
        if ($companies) {
            echo '<table class="shop_table"><thead><tr><th>Denumire</th><th>CUI</th><th>Adresă</th><th>Acțiuni</th></tr></thead><tbody>';
            foreach ($companies as $i => $c) {
                echo '<tr><td>'.esc_html($c['name']).'</td><td>'.esc_html($c['cui']).'</td><td>'.esc_html($c['address']).'</td><td><a href="#" class="edit-saved-company" data-index="'.$i.'">Editează</a> | <a href="#" class="delete-saved-company" data-index="'.$i.'">Șterge</a></td></tr>';
            }
            echo '</tbody></table>';
        } else echo '<p>Nu ai firme.</p>';
        
        echo '<button type="button" class="btn-add-small" id="add_new_company_btn">+ Adauga firma</button>';
        
        echo '<h3 style="margin-top:30px;">Persoane fizice</h3>';
        if ($persons) {
            echo '<table class="shop_table"><thead><tr><th>Nume</th><th>Tel</th><th>Email</th><th>Acțiuni</th></tr></thead><tbody>';
            foreach ($persons as $i => $p) {
                echo '<tr><td>'.esc_html($p['name']).'</td><td>'.esc_html($p['phone']).'</td><td>'.esc_html($p['email']).'</td><td><a href="#" class="edit-saved-person" data-index="'.$i.'">Editează</a> | <a href="#" class="delete-saved-person" data-index="'.$i.'">Șterge</a></td></tr>';
            }
            echo '</tbody></table>';
        } else echo '<p>Nu ai persoane.</p>';
        
        echo '<button type="button" class="btn-add-small" id="add_new_person_btn">+ Adauga persoana</button>';
    }
