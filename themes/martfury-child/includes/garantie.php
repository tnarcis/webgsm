...
// Adaugă tab-ul "Garanție" în My Account
...
add_action('init', function() {
    register_post_type('cerere_garantie', array(
        'labels' => array(
            'name' => 'Cereri Garanție',
            'singular_name' => 'Cerere Garanție',
            'menu_name' => 'Cereri Garanție',
...
add_action('woocommerce_account_garantie_endpoint', function() {
    ...
    if(isset($_GET['garantie_success'])) {
        echo '<div class="woocommerce-message">Cererea de garanție a fost trimisă cu succes! Vei fi contactat în curând.</div>';
    }
    ...
    if(isset($_POST['submit_garantie']) && wp_verify_nonce($_POST['garantie_nonce'], 'submit_garantie_form')) {
        ...
        if(empty($order_id) || empty($product_id) || empty($descriere) || $qty_garantie < 1) {
            ...
            if($qty_garantie > $qty_available) {
                echo '<div class="woocommerce-error">Nu poți trimite în garanție mai multe bucăți decât ai disponibile (' . $qty_available . ').</div>';
            }
            elseif(!produs_in_garantie($delivery_timestamp, $product_id)) {
                ...
                $garantie_id = wp_insert_post(array(
                    'post_type' => 'cerere_garantie',
                    'post_title' => 'Garanție #' . $order_id . ' - ' . date('Y-m-d H:i'),
                    'post_status' => 'publish',
    ...
    if(!empty($garantii)) {
        echo '<h3>Cererile tale de garanție</h3>';
    ...
    // Formularul de garanție nou
    echo '<h3>Solicită service în garanție</h3>';
    echo '<p><em>Perioada de garanție variază în funcție de tipul produsului și se calculează de la data livrării.</em></p>';
    ...
    if(empty($orders)) {
        echo '<p>Nu ai comenzi eligibile pentru garanție.</p>';
        return;
    }
    ...
    <form method="post" class="garantie-form" enctype="multipart/form-data">
        ...
        <p class="form-row" id="qty_garantie_row" style="display:none;">
            <label>Cantitate pentru garanție *</label>
        ...
        <p class="form-row">
            ...
            <input type="file" name="poze_garantie[]" multiple accept="image/jpeg,image/png,image/jpg" style="width:100%;">
            <small style="color:#666;">Pozele cu defectul ajută la procesarea mai rapidă a cererii tale.</small>
        </p>
        
        <p class="form-row">
            <button type="submit" name="submit_garantie" class="button">Trimite cererea de garanție</button>
        </p>
    </form>
    ...
    jQuery(document).ready(function($) {
        ...
        $('input[name="poze_garantie[]"]').on('change', function() {
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        if(response.data.length === 0) {
                            options = '<option value="">-- Nu sunt produse eligibile pentru garanție --</option>';
                        } else {
                            $.each(response.data, function(i, product) {
                                options += '<option value="' + product.id + '" data-garantie="' + product.garantie_luni + '" data-expira="' + product.data_expirare + '" data-qty="' + product.qty_available + '">' + product.name + ' - Garanție: ' + product.garantie_luni + ' luni (disponibil: ' + product.qty_available + ' buc)</option>';
                            });
                        }
        ...
        $('#select_product_garantie').on('change', function() {
            ...
            if(garantie) {
                $('#info_garantie').html('<strong>Garanție:</strong> ' + garantie + ' luni | <strong>Expiră:</strong> ' + expira).show();
            } else {
                $('#info_garantie').hide();
            }
...
add_action('woocommerce_product_data_panels', function() {
    ...
    woocommerce_wp_select(array(
        ...
        'label' => 'Perioadă garanție',
        'description' => 'Selectează perioada de garanție pentru acest produs',
        'desc_tip' => true,
        'options' => array(
            '0' => 'Fără garanție',
            '1' => '1 lună',
...
add_action('add_meta_boxes', function() {
    add_meta_box('garantie_details', 'Detalii Garanție', 'render_garantie_metabox', 'cerere_garantie', 'normal', 'high');
});
